...
def flash_fwd_kernel(
    ...
    for j in range(tl.cdiv( N_KEYS , K_TILE_SIZE)):
        ...
        Kj = tl.load(K_block_ptr, boundary_check=(0, 1), padding_option='zero') # K_tile_size x D
        Vj = tl.load(V_block_ptr, boundary_check=(0, 1), padding_option='zero')

        Sij = tl.dot(Qi , tl.trans(Kj)) * scale # Q_tile_size x K_tile_size

        m_ij = tl.max(Sij, 1)  # Reduction along axis 1
        mi_new = tl.maximum(mi, m_ij[:, None]) # Element-wise max
        
        # tl.device_print(mi_new, "mi_new:")
