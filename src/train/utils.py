...
def load_data(args, mode="train") -> (torch.utils.data.DataLoader, torch.utils.data.DataLoader):
    if mode == "train":
        ...
        train_dl = create_dataloader(train_paths, args.batch_size, args.context_length, device = args.device)
        val_dl = create_dataloader(val_paths, args.batch_size, args.context_length, device = args.device)
        return train_dl, val_dl
    elif mode == "test":
        test_paths = [os.path.join(args.test_path, f) for f in os.listdir(args.test_path)]
        test_dl = create_dataloader(test_paths, args.batch_size, args.context_length, device = args.device)
        return test_dl